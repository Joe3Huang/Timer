class Report
{
   ...
   //  	for (let i = 0; i < $scope.theModel.allMyEntries.length;i ++) 
   //  	{
   //  		let entry  = $scope.theModel.allMyEntries[i];
   //  		console.log(entry);
			// if (typeof(entry.phase) !== 'undefined')
			// {
			// 	result += entry.phase + ',';
			...
			// if (typeof(entry.tag) !== 'undefined')
			// {
			// result +=entry.tag.name + ',';
			// }
			// else
			// {
			// 	result += 'undefined' + ',';
			// }
   //  		result += entry.startTime.toString() + ',' + entry.interruptTime +',' + entry.stopTime.toString() + ',' + entry.deltaTime + "\n";
   //  	} 